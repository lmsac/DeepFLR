...
def combinerr(rawdataname,ms2framename):
    ...
    ms2frame.drop_duplicates(subset=['charge','key'],keep="first",inplace=True)
    ms2frame.columns=['repsequence', 'PP.Charge', 'decoration', 'ms2', 'key']
    def change_4(presequence):
        if "4" in presequence:
    ...
    # ms2frame=ms2frame.sort_values(by=["key","PP.Charge"],ascending=True,ignore_index=True)
    # rawdata=rawdata.sort_values(by=["key","PP.Charge"],ascending=True,ignore_index=True)

    rawdata["key"]=rawdata["key"].apply(change_4)
    # ms2frame=ms2frame.sort_values(by=["key","PP.Charge"],ascending=True,ignore_index=True)
    # rawdata=rawdata.sort_values(by=["key","PP.Charge"],ascending=True,ignore_index=True)
    # ccdata=pd.concat([rawdata,ms2frame],axis=1)

    ccdata=pd.merge(rawdata,ms2frame,on=["key","PP.Charge"],how="left")
    ccdata["charge"]=ccdata["PP.Charge"]
    ...
    def combine_in_memory():
        ...
        for index in range(N):
            ...
            if index % 10000==0:
                ...
                medianframe['i'] = medianframe['indexnumber'].apply(lambda x: x // 36)
                medianframe['j'] = medianframe['indexnumber'].apply(lambda x: x % 36)
                medianframe['FI.FrgType'] = medianframe["j"].apply(lambda x: ionname[x][0])
                medianframe['FI.Charge'] = medianframe["j"].apply(lambda x: ionname[x][-1])
                medianframe['FI.LossType'] = medianframe["j"].apply(lambda x: countloss(x))
                medianframe['FI.FrgNum'] = medianframe.apply(
                    lambda row: row['i'] + 1 if row['FI.FrgType'] == 'b' else row['length'] - row['i'] - 1,
                    axis=1)
                    
                medianframe["drop"] = medianframe.apply(drop, axis=1)
                medianframe = medianframe[~medianframe['drop'].isin([True])]
                medianframe.drop(['drop'], axis=1, inplace=True)
                medianframe.reset_index(drop=True, inplace=True)
...
def main():
    ...
    df=splitmakelibraryms2(combinerr(args.inputfile, args.repsequencefile),
